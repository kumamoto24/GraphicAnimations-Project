...
// Solution: H (6/16): Directional lights store the world-space travel direction used by all fragments.
...
void point_light_calculation(PointLightData point_light, LightCalculatioData calculation_data, float shininess, inout vec3 total_diffuse, inout vec3 total_specular, inout vec3 total_ambient) {
    ...
    //Solution: F (1/4)
    float distance_to_light = length(ws_light_offset);
    float attenuation = 1.0f / (1.0f + 0.15f * distance_to_light + 0.05f * distance_to_light * distance_to_light);

    // Ambient
    vec3 ambient_component = ambient_factor * point_light.colour * attenuation; // Solution: F (4/4)

    // Diffuse
    vec3 ws_light_dir = normalize(ws_light_offset);
    float diffuse_factor = max(dot(ws_light_dir, calculation_data.ws_normal), 0.0f);
    vec3 diffuse_component = diffuse_factor * point_light.colour * attenuation; //Solution: F (2/4)
    ...
    vec3 specular_component = specular_factor * point_light.colour * attenuation; //Solution: F (3/4)
...
LightingResult total_light_calculation(LightCalculatioData light_calculation_data, Material material
    ...
    #if NUM_PL + NUM_DL > 0
    // Solution: H (7/16): Average ambient over both point and directional lights so adding a new type stays balanced.
...
vec3 resolve_textured_light_calculation(LightingResult result, sampler2D diffuse_texture, sampler2D specular_map, vec2 texture_coordinate) {
    ...
    // Solution: E (4/10)
    vec2 scaled_texture_coordinate = texture_coordinate * texture_scale;

    // Solution: E (5/10) Updated following two variables
    vec3 texture_colour = texture(diffuse_texture, scaled_texture_coordinate).rgb;
    vec3 specular_map_sample = texture(specular_map, scaled_texture_coordinate).rgb;
