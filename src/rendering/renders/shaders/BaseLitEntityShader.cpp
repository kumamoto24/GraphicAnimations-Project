...
void BaseLitEntityShader::set_directional_lights(const std::vector<DirectionalLight>& directional_lights) {
    ...
    for (uint i = 0; i < count; i++) {
        ...
        // Solution: H (5/16): Upload normalised travel direction so shaders do not need pitch/yaw conversion.
        directional_lights_ubo.data[i].direction = glm::dot(directional_light.direction, directional_light.direction) > 0.000001f
                                                   ? glm::normalize(directional_light.direction)
