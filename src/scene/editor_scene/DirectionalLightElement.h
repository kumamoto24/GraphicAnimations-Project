...
namespace EditorScene {
    ...
    public:
        ...
        // Local visual anchor and local light travel direction.
        glm::vec3 position;
        glm::vec3 direction;
        bool visible = true;
        float visual_scale = 1.0f;
        std::shared_ptr<DirectionalLight> light;
        std::shared_ptr<EmissiveEntityRenderer::Entity> direction_indicator;

        DirectionalLightElement(const ElementRef& parent, std::string name, glm::vec3 position, glm::vec3 direction, std::shared_ptr<DirectionalLight> light, std::shared_ptr<EmissiveEntityRenderer::Entity> direction_indicator) :
            SceneElement(parent, std::move(name)), position(position), direction(direction), light(std::move(light)), direction_indicator(std::move(direction_indicator)) {}
